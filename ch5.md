...
- PathPattern 에서 리스코프 치환 원칙을 통해 다형성을 도와주며 개방 폐쇄 원칙을 지원하고 있는 것 같다 -> PathPattern 클래스 기능 확장에 열려있고 변경에는 닫혀있는 구조로!
- Enemy와 같은 컨트롤러의 역할을 하는 클래스에서는 단일 책임 원칙을 준수하는 구체적인 기능을 수행하는 객체들을 조립하는 방식을 사용하는 구조를 가질 수 있다고 생각
- Enemy : 고수준 모듈, EastMovePattern, SouthMovePattern : 저수준 모듈 -> 고수준, 저수준 모두 PathPattern에 의존함으로써, 고수준 모듈의 변경 없이 저수준 모듈을 변경할 수 있는 유연함을 얻을 수 있다?
- 리스코프 치환, 의존 역전 원칙은 단순히 클래스 상속, 인터페이스 구현의 차이??  
